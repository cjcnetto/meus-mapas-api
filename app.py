...
@app.post('/map', tags=[map_tag],
          responses={"200": UpsertMapResponse, "409": ErrorSchema, "400": ErrorSchema})
def add_map(form: UpsertMapRequest):
    """Insere um novo mapa ou atualiza um existente, se o id for definido como -1 ou não for passado um novo mapa é criado"""
    try:
...
def add_point(form: UpsertPointOfInterestRequest):
    """Insere um novo ponto de interesse ou atualiza um existente, dentro de um mapa, se o id for definido como -1 ou não for passado um novo ponto de interesse é criado"""
    try:
